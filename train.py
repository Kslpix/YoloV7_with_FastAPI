...
from yolo_nets.training import (ModelEMA, YOLOLoss, get_lr_scheduler,
                                set_optimizer_lr, weights_init)
from yolo_utils.callbacks import EvalCallback, LossHistory
from yolo_utils.dataloader import YoloDataset, yolo_dataset_collate
from yolo_utils.utils import download_weights, get_anchors, get_classes, show_config
...
2、损失值的大小用于判断是否收敛,比较重要的是有收敛的趋势,即验证集损失不断下降,如果验证集损失基本上不改变的话,模型基本上就收敛了。
   ...
   训练过程中的损失值会保存在output_train文件夹下的loss_%Y_%m_%d_%H_%M_%S文件夹中
   
3、训练好的权值文件保存在output_train文件夹中,每个训练世代(Epoch)包含若干训练步长(Step),每个训练步长(Step)进行一次梯度下降。
...
if __name__ == "__main__":
    ...
    fp16            = False
    #---------------------------------------------------------------------#
    #   classes_path    指向model_train下的txt,与自己训练的数据集相关 
    #                   训练前一定要修改classes_path,使其对应自己的数据集
    #---------------------------------------------------------------------#
    classes_path    = 'model_train/voc_classes.txt'
    #---------------------------------------------------------------------#
    #   anchors_path    代表先验框对应的txt文件,一般不修改。
    #   anchors_mask    用于帮助代码找到对应的先验框,一般不修改。
    #---------------------------------------------------------------------#
    anchors_path    = 'model_train/yolo_anchors.txt'
    ...
    #
    #   如果训练过程中存在中断训练的操作,可以将model_path设置成output_logs文件夹下的权值文件,将已经训练了一部分的权值再次载入。
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = 'model_train/yolov7_weights.pth'
    #------------------------------------------------------#
    #   input_shape     输入的shape大小,一定要是32的倍数
    #------------------------------------------------------#
    input_shape     = [640, 640]
    #------------------------------------------------------#
    #   phi             所使用到的yolov7的版本,本仓库一共提供三个：
    #                   l : 对应yolov7
    #                   x : 对应yolov7_x
    #                   t : 对应yolov7_tiny
    #------------------------------------------------------#
    ...
    #------------------------------------------------------#
    train_annotation_path   = 'model_train/2007_train.txt'
    val_annotation_path     = 'model_train/2007_val.txt'
    #------------------------------------------------------#
    #   设置用到的显卡
    #------------------------------------------------------#
